<template>
  ...
  <nav>    
    ...
    <li><router-link to="/staff">اضافة موظف</router-link></li>
  ...
  <div v-if="staff.length">
    <div v-for="employee in staff" :key="employee.id">
      <SingleStaff :employee="employee" @delete="handleDelete" />
    </div>
  </div>
</base-card>
...
import SingleStaff from "./SingleStaff.vue";
export default {
  
  components: { SingleStaff , BaseCard},
  data() {
    return {
      staff: [],
      current: "all",
  ...
  methods: {
    handleDelete(id) {
      this.staff = this.staff.filter((employee) => {
