  ...
  <form @submit.prevent="handleSubmit">
      ...
      <div class="user-box">
        ...
        <br>
			...
			<div class="user-box">
				<select name="postion" v-model="postion" required="">
					<option value="">اختر الوصف الوظيفي</option>
					<option value="مسؤول مختبر بنك الدم">مسؤول مختبر بنك الدم</option>
...
export default {
  ...
  data() {
    return {
      uri: "http://localhost:3000/staff/" + this.id,
      FullName: "",
      phoneNumber: "",
      Age: "",
      email: "",
  ...
  methods: {
    handleSubmit() {
      fetch(this.uri, {
        ...
        body: JSON.stringify({ FullName: this.FullName, phoneNumber: this.phoneNumber,Age:this.Age,email:this.email,address:this.address, postion:this.postion}),
      })
        .then(() => {
