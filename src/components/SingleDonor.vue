<template>
  <div class="actions">
    <h3 @click="showDetails = !showDetails">اسم المتبرع:{{ donor.FullName }}</h3>
    ...
    <p>{{ donor.phoneNumber }}</p>
        <p>{{ donor.Age }}</p>
            <p>{{ donor.email }} </p>
                ...
                <p>عنوان المتبرع:{{ donor.address }}</p>

                    <div class="icons">
      <button @click="deletedonors" class="material-icons">حذف</button>
      <router-link :to="{ name: 'EditDonor', params: { id: donor.id } }">
...
export default {
  ...
  data() {
    return {
       showDetails: false,
      uri: "http://localhost:3000/donors/" + this.donor.id,
    };
  },
  methods: {
    deletedonors() {
      fetch(this.uri, { method: "DELETE" })
        .then(() => this.$emit("delete", this.donor.id))
